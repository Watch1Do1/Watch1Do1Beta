...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    if (view === 'profile' && currentUser) return <ProfilePage user={currentUser} currentUser={currentUser} userVideos={videos.filter(v => v.creatorId.toLowerCase() === currentUser.email.toLowerCase())} scannedVideos={videos.filter(v => currentUser.scannedVideoIds?.includes(v.id))} allVideos={videos} onAvatarChange={(file) => { const reader = new FileReader(); reader.onload = async (e) => { const base64Url = e.target?.result as string; if (currentUser) { const updated = {...currentUser, avatarUrl: base64Url}; await dbService.upsertUser(updated); setCurrentUser(updated); } }; reader.readAsDataURL(file); }} onVideoClick={handleVideoSelect} onProfileUpdate={async (info) => { if (!currentUser) return; const updated = {...currentUser, ...info}; await dbService.upsertUser(updated); setCurrentUser(updated); }} onManageSubscription={() => setManageSubscriptionModalOpen(true)} onBack={handleNavigateHome} onShare={handleTriggerShare} defaultTab={profileInitialTab} />;
